...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Genoma runs”).
...
- `genoma health` and `genoma status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `genoma gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`GENOMA_GATEWAY_PORT`).
...
- Local mode defaults:
  - token: `GENOMA_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token`
  - password: `GENOMA_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password`
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `GENOMA_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
