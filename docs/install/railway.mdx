...
Deploy Genome on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/genome-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/genome` — Control UI

## What you get

- Hosted Genome Gateway + Control UI
...
- `GENOME_STATE_DIR=/data/.genome` (recommended)
- `GENOME_WORKSPACE_DIR=/data/workspace` (recommended)
- `GENOME_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Genome state + workspace so you can migrate to another host without losing config or memory.
