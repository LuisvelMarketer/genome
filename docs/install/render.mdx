...
Deploy Genome on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/genome/genome)
...
repository configures everything needed to run Genome:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: GENOME_STATE_DIR
        value: /data/.genome
      - key: GENOME_WORKSPACE_DIR
        value: /data/workspace
      - key: GENOME_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/genome`.
...
If you use the original Genome repository, Render will not auto-deploy your Genome. To update it, run a manual Blueprint sync from the dashboard.
...
For Genome, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Genome host.
