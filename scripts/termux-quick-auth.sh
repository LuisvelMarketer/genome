...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/genoma/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Genoma Re-Auth" -c "After getting key, run: ssh $SERVER '~/genoma/scripts/mobile-reauth.sh'" --id genoma-auth
        ;;
    *)
