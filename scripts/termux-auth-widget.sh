...
# Check auth status
termux-toast "Checking Genoma auth..."

STATUS=$(ssh "$SERVER" '$HOME/genoma/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/genoma/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|GENOMA_EXPIRING|GENOMA_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/genoma/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|GENOMA_EXPIRED|GENOMA_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Genoma won't work until re-auth"
                ;;
        esac
        ;;
