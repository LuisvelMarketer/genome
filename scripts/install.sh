...
# Genoma Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Genoma."
...
GUM_VERSION="${GENOMA_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Genoma Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://genoma.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$GENOMA_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.genoma.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_genoma_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.genoma-* "$npm_root"/genoma 2>/dev/null || true
}

extract_genoma_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_genoma_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/genoma" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/genoma"|"/usr/local/bin/genoma")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/genoma/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale genoma symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing genoma binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Genoma package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_genoma_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Genoma npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*genoma" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_genoma_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_genoma_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Genoma npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an genoma binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${GENOMA_TAGLINE_INDEX:-}" ]]; then
        if [[ "${GENOMA_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((GENOMA_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${GENOMA_NO_ONBOARD:-0}
NO_PROMPT=${GENOMA_NO_PROMPT:-0}
DRY_RUN=${GENOMA_DRY_RUN:-0}
INSTALL_METHOD=${GENOMA_INSTALL_METHOD:-}
GENOMA_VERSION=${GENOMA_VERSION:-latest}
USE_BETA=${GENOMA_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/genoma"
GIT_DIR=${GENOMA_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${GENOMA_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${GENOMA_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${GENOMA_VERBOSE:-0}"
GENOMA_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Genoma installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/genoma)
...
Environment variables:
  GENOMA_INSTALL_METHOD=git|npm
  GENOMA_VERSION=latest|next|<semver>
  GENOMA_BETA=0|1
  GENOMA_GIT_DIR=...
  GENOMA_GIT_UPDATE=0|1
  GENOMA_NO_PROMPT=1
  GENOMA_DRY_RUN=1
  GENOMA_NO_ONBOARD=1
  GENOMA_VERBOSE=1
  GENOMA_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Genoma checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Genoma source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_genoma_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"genoma"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://genoma.ai/install.sh | bash"
}
...
ensure_genoma_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/genoma" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/genoma" ]]; then
        ln -sf "$npm_root/genoma/dist/entry.js" "${npm_bin}/genoma"
        ui_info "Created genoma bin link at ${npm_bin}/genoma"
    fi
    return 0
}

# Check for existing Genoma installation
check_existing_genoma() {
    if [[ -n "$(type -P genoma 2>/dev/null || true)" ]]; then
        ui_info "Existing Genoma installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make genoma show as \"command not found\" in new terminals."
...
warn_genoma_not_found() {
    ui_warn "Installed, but genoma is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t genoma 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named genoma; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_genoma_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P genoma 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/genoma" ]]; then
        echo "${npm_bin}/genoma"
        return 0
    fi
...
install_genoma_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/genoma/genoma.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Genoma from git checkout: ${repo_dir}"
    else
        ui_info "Installing Genoma from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Genoma" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Genoma" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/genoma" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/genoma"
    ui_success "Genoma wrapper installed to \$HOME/.local/bin/genoma"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view genoma dist-tags.beta 2>/dev/null || true)"
...
install_genoma() {
    local package_name="genoma"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            GENOMA_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="genoma"
        else
            GENOMA_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${GENOMA_VERSION}" ]]; then
        GENOMA_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${GENOMA_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Genoma v${resolved_version}"
    else
        ui_info "Installing Genoma (${GENOMA_VERSION})"
    fi
    local install_spec=""
    if [[ "${GENOMA_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${GENOMA_VERSION}"
    fi

    if ! install_genoma_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_genoma_paths
        install_genoma_npm "${install_spec}"
    fi

    if [[ "${GENOMA_VERSION}" == "latest" && "${package_name}" == "genoma" ]]; then
        if ! resolve_genoma_bin &> /dev/null; then
            ui_warn "npm install genoma@latest failed; retrying genoma@next"
            cleanup_npm_genoma_paths
            install_genoma_npm "genoma@next"
        fi
    fi

    ensure_genoma_bin_link || true

    ui_success "Genoma installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${GENOMA_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_genoma_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (genoma not on PATH yet)"
        warn_genoma_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${GENOMA_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.genoma/workspace-${profile}"
    else
        echo "${HOME}/.genoma/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run genoma onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but genoma not on PATH; skipping onboarding"
        warn_genoma_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run genoma onboard to retry"
        return
    }
}

resolve_genoma_version() {
    local version=""
    local claw="${GENOMA_BIN:-}"
    if [[ -z "$claw" ]] && command -v genoma &> /dev/null; then
        claw="$(command -v genoma)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/genoma/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/genoma/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_genoma_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Genoma checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set GENOMA_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_genoma_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/genoma" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/genoma"
    ...
    GENOMA_BIN="$(resolve_genoma_bin || true)"
    ...
    installed_version=$(resolve_genoma_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Genoma installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Genoma installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/genoma"
        ui_kv "Update command" "genoma update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://genoma.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running genoma doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                GENOMA_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                GENOMA_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                GENOMA_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run genoma doctor and genoma plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (genoma not on PATH yet)"
                    warn_genoma_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run genoma onboard to finish setup"
            return 0
        fi
    fi

    if command -v genoma &> /dev/null; then
        local claw="${GENOMA_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_genoma_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (genoma daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if GENOMA_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: genoma daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${GENOMA_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
