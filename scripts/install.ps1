# Genome Installer for Windows (PowerShell)
# Usage: iwr -useb https://genome.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://genome.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\genome",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🧬 Genome Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Genome.$NC" -Level info
    Write-Host ""
}
...
function Install-GenomeNpm {
    param([string]$Version = "latest")

    Write-Host "Installing Genome (genome@$Version)..." -Level info

    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g genome@$Version --no-fund --no-audit 2>&1
        Write-Host "Genome installed" -Level success
        return $true
    } catch {
...
function Install-GenomeGit {
    param([string]$RepoDir, [switch]$Update)

    Write-Host "Installing Genome from git..." -Level info

    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/genome/genome.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\genome\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\genome.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Genome from git to $GitDir" -Level info
        } else {
            Install-GenomeGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Genome via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-GenomeNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'genome onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🧬 Genome installed successfully!" -Level success
}
