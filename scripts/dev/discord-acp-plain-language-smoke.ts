...
type DriverMode = "token" | "webhook" | "genoma";
...
function resolveStateDir(): string {
  const override = process.env.GENOMA_STATE_DIR?.trim() || process.env.GENOMA_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.GENOMA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".genoma");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver genoma] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Genoma spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|genoma> Driver transport mode (default: token)\n" +
    ...
    "  --genoma-bin <path>        Genoma CLI binary for driver=genoma (default: genoma)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  GENOMA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  GENOMA_DISCORD_SMOKE_DRIVER\n" +
    "  GENOMA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  GENOMA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  GENOMA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  GENOMA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  GENOMA_DISCORD_SMOKE_AGENT\n" +
    "  GENOMA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  GENOMA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  GENOMA_DISCORD_SMOKE_POLL_MS\n" +
    "  GENOMA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  GENOMA_DISCORD_SMOKE_GENOMA_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.GENOMA_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.GENOMA_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.GENOMA_DISCORD_SMOKE_DRIVER ||
    process.env.GENOMA_DISCORD_SMOKE_DRIVER ||
    "token";
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.GENOMA_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.GENOMA_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.GENOMA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.GENOMA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.GENOMA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.GENOMA_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.GENOMA_DISCORD_SMOKE_AGENT ||
    process.env.GENOMA_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.GENOMA_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.GENOMA_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.GENOMA_DISCORD_SMOKE_INSTRUCTION ||
    process.env.GENOMA_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.GENOMA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.GENOMA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.GENOMA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const genomaBin =
    resolveArg("--genoma-bin") || process.env.GENOMA_DISCORD_SMOKE_GENOMA_BIN || "genoma";
...
async function genomaCliJson<T>(params: { genomaBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.genomaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`genoma ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await genomaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    genomaBin: params.genomaBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "genoma") {
    return await readMessagesWithOpenclaw({
      genomaBin: params.args.genomaBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `genoma-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await genomaCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("genoma message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Genoma.`,
        diagnostics: {
          bindingCandidates: [
            {
