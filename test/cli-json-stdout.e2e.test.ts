...
describe("cli json stdout contract", () => {
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".genome");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "genome.json"), "{}", "utf8");
        ...
        delete env.GENOMA_HOME;
        delete env.GENOMA_STATE_DIR;
        delete env.GENOMA_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "genoma.mjs");
