...
```bash
cat ~/.genoma/genoma.json | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:18793/__genoma__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__genoma__/canvas/<file>.html`
...
1. Check server bind: `cat ~/.genoma/genoma.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__genoma__/canvas/<file>.html`
...
Node is offline. Use `genoma nodes list` to find online nodes.
...
The canvas host serves from `/__genoma__/canvas/` prefix:

```
http://<host>:18793/__genoma__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:18793/__genoma__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__genoma__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
