...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.GENOMA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genoma-nostr-"));
  process.env.GENOMA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.GENOMA_STATE_DIR?.trim() || stateEnv.GENOMA_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".genoma");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      delete process.env.GENOMA_STATE_DIR;
    } else {
      process.env.GENOMA_STATE_DIR = previous;
    }
