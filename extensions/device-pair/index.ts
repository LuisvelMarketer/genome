...
function resolveAuth(cfg: GenomaPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: GenomaPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Genoma iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Genoma iOS app:",
            "",
            "```",
            qrAscii,
            "```",
