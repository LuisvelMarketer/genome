...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.GENOMA_STATE_DIR?.trim() || env.GENOMA_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["genoma-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".genoma");
}
