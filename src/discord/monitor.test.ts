...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Genoma")).toBe("friends-of-genoma");
  ...
  it("matches ids by default and names only when enabled", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "LuisvelMarketer", "Friends of Genoma"],
    ...
    expect(allowListMatches(allow, { name: "LuisvelMarketer" })).toBe(false);
    expect(allowListMatches(allow, { name: "friends-of-genoma" })).toBe(false);
    expect(allowListMatches(allow, { name: "LuisvelMarketer" }, { allowNameMatching: true })).toBe(
      true,
    );
    expect(
      allowListMatches(allow, { name: "friends-of-genoma" }, { allowNameMatching: true }),
    ).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-genoma" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Genoma"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-genoma");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-genoma": { slug: "friends-of-genoma" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Genoma"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["genoma-dm"],
        channelId: "1",
        channelName: "Genoma DM",
        channelSlug: "genoma-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
