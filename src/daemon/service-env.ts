...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.GENOMA_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    GENOMA_PROFILE: profile,
    GENOMA_GATEWAY_PORT: String(port),
    GENOMA_GATEWAY_TOKEN: token,
    GENOMA_LAUNCHD_LABEL: resolvedLaunchdLabel,
    GENOMA_SYSTEMD_UNIT: systemdUnit,
    GENOMA_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    GENOMA_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    GENOMA_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.GENOMA_GATEWAY_TOKEN?.trim() || env.GENOMA_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    GENOMA_GATEWAY_TOKEN: gatewayToken,
    GENOMA_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    GENOMA_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    GENOMA_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    GENOMA_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    GENOMA_LOG_PREFIX: "node",
    GENOMA_SERVICE_MARKER: NODE_SERVICE_MARKER,
    GENOMA_SERVICE_KIND: NODE_SERVICE_KIND,
    GENOMA_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  return {
    ...
    GENOMA_STATE_DIR: sharedEnv.stateDir,
    GENOMA_CONFIG_PATH: sharedEnv.configPath,
  };
}
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.GENOMA_STATE_DIR;
  const configPath = env.GENOMA_CONFIG_PATH;
