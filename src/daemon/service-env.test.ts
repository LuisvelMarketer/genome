...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.GENOMA_GATEWAY_PORT).toBe("18789");
    expect(env.GENOMA_GATEWAY_TOKEN).toBe("secret");
    expect(env.GENOMA_SERVICE_MARKER).toBe("genoma");
    expect(env.GENOMA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.GENOMA_SERVICE_VERSION).toBe("string");
    expect(env.GENOMA_SYSTEMD_UNIT).toBe("genoma-gateway.service");
    if (process.platform === "darwin") {
      expect(env.GENOMA_LAUNCHD_LABEL).toBe("ai.genoma.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", GENOMA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.GENOMA_SYSTEMD_UNIT).toBe("genoma-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.GENOMA_LAUNCHD_LABEL).toBe("ai.genoma.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through GENOMA_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GENOMA_GATEWAY_TOKEN: " node-token " },
    });
  ...
  it("omits GENOMA_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genoma"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", GENOMA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genoma-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", GENOMA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genoma"));
  });

  it("uses GENOMA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", GENOMA_STATE_DIR: "/var/lib/genoma" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/genoma"));
  });

  it("expands ~ in GENOMA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", GENOMA_STATE_DIR: "~/genoma-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/genoma-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { GENOMA_STATE_DIR: "C:\\State\\genoma" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\genoma");
  });
});
