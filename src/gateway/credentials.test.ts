...
function cfg(input: Partial<GenomaConfig>): GenomaConfig {
  return input as GenomaConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<GenomaConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  GENOMA_GATEWAY_TOKEN: "env-token",
  GENOMA_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy GENOME env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        GENOMA_GATEWAY_TOKEN: "legacy-token",
        GENOMA_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
      includeLegacyEnv: false,
    });
