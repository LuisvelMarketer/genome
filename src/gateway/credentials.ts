...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.GENOMA_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.GENOMA_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.GENOMA_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.GENOMA_GATEWAY_PASSWORD);
}
