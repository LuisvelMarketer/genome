...
export function buildGatewayConnectionDetails(
  ...
  const allowPrivateWs = process.env.GENOMA_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set GENOMA_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: genoma doctor --fix",
        "Docs: https://docs.genoma.ai/gateway/remote",
      ].join("\n"),
    );
  }
