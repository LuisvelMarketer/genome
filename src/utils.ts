...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".genoma");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.GENOMA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$GENOMA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via GENOMA_STATE_DIR.
