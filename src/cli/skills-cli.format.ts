...
function appendGenomeHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("genoma skills list")}\` to see all skills.`
      : "No skills found.";
    return appendGenomeHint(message, opts.json);
  }
  ...
  return appendGenomeHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendGenomeHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("genoma skills list")}\` to see available skills.`,
      opts.json,
    );
  }
