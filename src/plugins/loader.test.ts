...
import { __testing, loadGenomaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "genoma-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.GENOMA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "genoma.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.GENOMA_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadGenomaPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          genoma: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.GENOMA_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.GENOMA_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadGenomaPlugins>) {
...
function useNoBundledPlugins() {
  process.env.GENOMA_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadGenomaPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "genoma.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.GENOMA_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.GENOMA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadGenomaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadGenomaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadGenomaPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadGenomaPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ GENOMA_STATE_DIR: stateDir }, () => {
