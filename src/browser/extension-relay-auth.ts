...
const RELAY_TOKEN_CONTEXT = "genoma-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const GENOMA_RELAY_BROWSER = "Genoma/extension-relay";
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or GENOMA_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedGenomaRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === GENOMA_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
